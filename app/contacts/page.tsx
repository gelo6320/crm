...
function ContactDetailModal({ contact, onClose }: ContactDetailModalProps) {
  ...
  // Gestisci l'animazione di apertura con timing più lungo per il blur
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsOpening(false);
    }, 50); // Aumentato da 10ms a 50ms per dare tempo al DOM
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ${isClosing || isOpening ? 'opacity-0' : 'opacity-100'}`}>
      {/* Backdrop con blur separato per migliore controllo dell'animazione */}
      <div 
        className={`absolute inset-0 bg-black/40 transition-all duration-500 ${
          isClosing || isOpening ? 'backdrop-blur-none' : 'backdrop-blur-sm'
        }`}
        onClick={handleClose}
      ></div>
      
      {/* Modal con frosted glass - SOLUZIONE 1: Rimuovere la condizione per backdrop-blur */}
      <div className={`relative z-10 w-full max-w-lg mx-6 transition-all duration-300 ${isClosing || isOpening ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        {/* Modal frosted glass background - Blur sempre attivo per evitare il salto */}
        <div className="relative bg-zinc-50/60 dark:bg-zinc-100/5 backdrop-blur-xl rounded-[24px] border border-white/30 dark:border-white/20 shadow-lg overflow-hidden">
          {/* Overlay aggiuntivo per animazione opacità del contenuto */}
          <div className={`transition-opacity duration-400 ${isClosing || isOpening ? 'opacity-0' : 'opacity-100'}`}>
            {/* Content */}
